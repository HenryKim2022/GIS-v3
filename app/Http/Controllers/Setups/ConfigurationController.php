...
class ConfigurationController extends Controller
{
    ...
    {
        return view('/setups/v_configure');
    }

    public function showTemplatePage()
    {
        if (env(key: 'APP_ENV') == 'local'){
            return redirect(base_url("public/materialize"));
        }else{
            return redirect("public/materialize");
        }
    }
    ...
    public function saveConfiguration(Request $request)
    {
        if ($request != null){
            ...
            $envData = [
                ...
                'APP_YEAR' => $request->input('app_year') == 'null' ? null : '"' . $request->input('app_year') . '"',
                ...
                'APP_INSTITUTION' => $request->input('app_institution') == 'null' ? null : '"' . $request->input('app_institution') . '"',
                'APP_PURPOSE' => $request->input('app_purpose') == 'null' ? null : '"' . $request->input('app_purpose') . '"',
                'GROUP_MEMBER' => env(key: 'GROUP_MEMBER') == 'null' ? $groupMembers : $groupMembers . "\n" . "\n",
            ...
            return redirect('/setups/v_configure');
